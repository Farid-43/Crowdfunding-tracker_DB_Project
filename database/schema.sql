-- =====================================================
-- CF_Tracker Database Schema
-- Educational SQL Feature Demonstration Platform
-- =====================================================
-- This schema demonstrates EVERY SQL feature from Oracle curriculum
-- adapted for MySQL/MariaDB compatibility

-- Drop existing database and recreate (for clean setup)
DROP DATABASE IF EXISTS CF_Tracker;
CREATE DATABASE CF_Tracker CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE CF_Tracker;
...
GROUP BY c.campaign_id, c.campaign_title, c.category, c.goal_amount, c.current_amount, 
...
BEGIN
    ...
    INSERT INTO Query_Log (query_text, query_type, page_name, notes)
    VALUES (CONCAT('Campaign deleted: ', OLD.campaign_title), 'DELETE', 'TRIGGER', 
            CONCAT('Campaign ID: ', OLD.campaign_id, ' deleted by user'));
END//
